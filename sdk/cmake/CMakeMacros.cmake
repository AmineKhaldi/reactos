...
function(enable_unity_build _target_name _source_variable_name)
    set(_source_files ${${_source_variable_name}})
    if(IS_CPP)
        set(_extension "cpp")
    else()
        set(_extension "c")
    endif()
    set(_unity_build_file ${CMAKE_CURRENT_BINARY_DIR}/${_target_name}_unity.${_extension})
    set(_temp_unity_build_file ${_unity_build_file}.tmp)
    set_source_files_properties(${_source_files} PROPERTIES HEADER_FILE_ONLY TRUE)
    file(WRITE ${_temp_unity_build_file} "/* ReactOS unity build file for ${_target_name} */\n")
    foreach(_source_file ${_source_files})
        file(APPEND ${_temp_unity_build_file} "#include <${_source_file}>\n")
    endforeach()
    configure_file(${_temp_unity_build_file} ${_unity_build_file} COPYONLY)
    file(REMOVE ${_temp_unity_build_file})
    target_sources(${_target_name} PRIVATE ${_unity_build_file})
endfunction()
