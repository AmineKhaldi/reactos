...
{
    PrintDefaultUsage(_T(" VOLUME "), Command, (HandlerItem *)&VolumeHandlersList,
                      (sizeof(VolumeHandlersList) / sizeof(VolumeHandlersList[0])));
}

int
VolumeMain(int argc, const TCHAR *argv[])
{
    return FindHandler(argc, argv, (HandlerItem *)&VolumeHandlersList,
                       (sizeof(VolumeHandlersList) / sizeof(VolumeHandlersList[0])),
                       VolumePrintUsage);
}
