...
{
    PrintDefaultUsage(_T(" DIRTY "), Command, (HandlerItem *)&DirtyHandlersList,
                      (sizeof(DirtyHandlersList) / sizeof(DirtyHandlersList[0])));
}

int
DirtyMain(int argc, const TCHAR *argv[])
{
    return FindHandler(argc, argv, (HandlerItem *)&DirtyHandlersList,
                       (sizeof(DirtyHandlersList) / sizeof(DirtyHandlersList[0])),
                       DirtyPrintUsage);
}
