...
{
    PrintDefaultUsage(_T(" HARDLINK "), Command, (HandlerItem *)&HardLinkHandlersList,
                      (sizeof(HardLinkHandlersList) / sizeof(HardLinkHandlersList[0])));
}

int
HardLinkMain(int argc, const TCHAR *argv[])
{
    return FindHandler(argc, argv, (HandlerItem *)&HardLinkHandlersList,
                       (sizeof(HardLinkHandlersList) / sizeof(HardLinkHandlersList[0])),
                       HardLinkPrintUsage);
}
